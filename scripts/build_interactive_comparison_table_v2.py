...
<style>
  ...
  * {{ box-sizing: border-box; margin: 0; padding: 0; }}
  ...
  .action-btns {{ display: flex; gap: 2px; align-items: center; margin-left: 6px; }}
...
<div class="controls">
  <div class="note" style="margin-top:0">SPOT-Trankit only. Click a row for examples. <kbd>↑↓</kbd> to navigate, <kbd>Esc</kbd> to close.</div>
  <div>
...
<div class="controls">
  <div class="note" style="margin-top:0">SPOT-Trankit only. Click a row for examples.</div>
...
<div class="controls">
  <div class="note" style="margin-top:0">SPOT-Trankit only. Hover a row for MSD decode. Click for examples.</div>
  <div style="display:flex;gap:8px;align-items:center">
...
(function () {{
  ...
  function renderExamples(items, titleText) {{
  ...
  function paintDeprel(bucket) {{
    ...
    if (tbody._countNote) tbody._countNote.textContent = `Showing ${{visible.length}} / ${{filtered.length}}`;
    if (filtered.length <= ROW_LIMIT) {{ toggle.hidden = true; }}
    else {{ toggle.hidden = false; toggle.textContent = expanded ? 'Show less' : `Show more (${{filtered.length - ROW_LIMIT}})`; }}
  }}
  ...
  for (const bucket of ['head_only', 'both_wrong', 'rel_only']) {{
    ...
    countBar.appendChild(countNote);
  ...
  function makeTagErrTable(tbodyId, countNoteId, toggleId, mergeBtnId, filterInputId, layer) {{
    ...
    function render() {{
      ...
      for (const row of visible) {{
        ...
        if (layer === 'xpos') {{
          ...
          labelCell.setAttribute('data-tip', msdTip(row.label));
        }}
        ...
        tr.addEventListener('click', function() {{
          ...
          renderExamples(exs, `${{layer.toUpperCase()}} · ${{row.label}} (${{exs.length}} examples)`);
        }});
        tbody.appendChild(tr);
      }}
    ...
    if (csvBtn) csvBtn.addEventListener('click', function() {{
      const allRows = (data.tag_error_rows[layer] || []);
      downloadCSV(['Error pair','Count'], allRows.map(function(r) {{ return [r.label, r.count]; }}), layer + '-errors.csv');
    }});
