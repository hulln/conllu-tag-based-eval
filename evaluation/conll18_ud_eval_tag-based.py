...
def evaluate(gold_ud, system_ud, xpos=None, upos=None, uas=None, las=None):
    class Score:
    ...
    def alignment_score(alignment, key_fn=None, filter_fn=None, filter_val=None):
        if filter_fn is not None:
            if filter_val is not None:
                gold = sum(1 for gold in alignment.gold_words if filter_fn(gold, filter_val))
                system = sum(1 for system in alignment.system_words if filter_fn(system, filter_val))
                aligned = sum(1 for word in alignment.matched_words if filter_fn(word.gold_word, filter_val))
                #system = sum(1 for system in alignment.system_words if filter_fn(system, filter_val))
                #aligned = sum(1 for word in alignment.matched_words if filter_fn(word.gold_word, filter_val))
            else:
        ...
        for words in alignment.matched_words:
            if filter_val is not None:
                if filter_fn is None or filter_fn(words.gold_word, filter_val):
    ...
    def get_unquie_col_vals(alignment, col):
        uniques = set()
        for word in alignment.gold_words + alignment.system_words:
            val = word.columns[col]
            uniques.add(val)
    ...
    unique_deprel = get_unquie_col_vals(alignment, DEPREL)
    if uas:
        matches = []
        for val in unique_deprel:
            if re.match(uas, val):
                matches.append(val)

        for val in matches:
            res["UAS_{}".format(val)] = alignment_score(alignment, 
                    key_fn=lambda w, ga: ga(w.parent),
                    filter_fn=lambda w, val: w.columns[DEPREL] == val, 
                    filter_val=val)

        if len(matches) > 0:
            res["UAS_{}".format(uas)] = alignment_score(alignment, 
                    key_fn=lambda w, ga: ga(w.parent),
                    filter_fn=lambda w: w.columns[DEPREL] in matches)
    ...
    if las:
        ...
        for val in matches:
            res["LAS_{}".format(val)] = alignment_score(alignment, 
                    key_fn=lambda w, ga: (ga(w.parent), w.columns[DEPREL]),
                    filter_fn=lambda w, val: w.columns[DEPREL] == val, 
                    filter_val=val)

        if len(matches) > 0:
            res["LAS_{}".format(las)] = alignment_score(alignment, 
                    key_fn=lambda w, ga: ga(w.parent),
                    filter_fn=lambda w: w.columns[DEPREL] in matches)

    if upos:
        matches = []
        for val in get_unquie_col_vals(alignment, UPOS):
            if re.match(upos, val):
                matches.append(val)

        for val in matches:
            res["UPOS_{}".format(val)] = alignment_score(alignment, 
                    key_fn=lambda w, _: w.columns[UPOS],
                    filter_fn=lambda w, val: w.columns[UPOS] == val, 
                    filter_val=val)

        if len(matches) > 0:
            res["UPOS_{}".format(upos)] = alignment_score(alignment, 
                    key_fn=lambda w, _: w.columns[UPOS],
                    filter_fn=lambda w: w.columns[UPOS] in matches)

    if xpos:
        matches = []
        for val in get_unquie_col_vals(alignment, XPOS):
            if re.match(xpos, val):
                matches.append(val)

        for val in matches:
                res["XPOS_{}".format(val)] = alignment_score(alignment, 
                        key_fn=lambda w, _: w.columns[XPOS],
                        filter_fn=lambda w, val: w.columns[XPOS] == val, 
                        filter_val=val)

        if len(matches) > 0:
            res["XPOS_{}".format(xpos)] = alignment_score(alignment, 
                    key_fn=lambda w, _: w.columns[XPOS],
                    filter_fn=lambda w: w.columns[XPOS] in matches)
...
def evaluate_wrapper(args):
    ...
    return evaluate(gold_ud, system_ud, xpos=args.xpos, upos=args.upos, las=args.las, uas=args.uas)

def print_metric_results(evaluation, metric, counts):
    if counts:
...
def main():
    ...
    parser.add_argument("--xpos", type=str, default=None,
                        help="Show statistics of XPOS prediction accuracy for each tag seperately. Show results based on the passed regex.\
                                For example: --xpos \"N*\"")
    parser.add_argument("--upos", type=str, default=None,
                        help="Show statistics of UPOS prediction accuracy for each tag seperately. Show results based on the passed regex.\
                                For example: --upos \"N*\"")
    parser.add_argument("--uas", type=str, default=None,
                        help="Filter DEPREL values for calculation of the UAS statistic. Show results based on the passed regex.\
                                For example: --uas \"N*\"")
    parser.add_argument("--las", type=str, default=None,
                        help="Filter DEPREL values for calculation of the LAS statistic. Show results based on the passed regex.\
                                For example: --las \"N*\"")
    ...
    if not args.verbose and not args.counts:
        ...
        #    print_metric_results(evaluation , metric, args.counts)
        #    if metric == "UPOS":
        #        for val in UPOS_VALS:
        #            print_metric_results(evaluation , '{}_{}'.format(metric, val), args.counts)
        #    if metric == "XPOS":
        #        for val in XPOS_VALS:
        #            print_metric_results(evaluation , '{}_{}'.format(metric, val), args.counts)
